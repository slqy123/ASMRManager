...
class ASMRDownloadAPI(ASMRAPI):
    ...
    async def download(
        ...
        logger.debug(f"file list: {file_list_with_order}")
        for download_order in sorted(
            ...
            logger.error("No audio file found to download, skip this voice")
            return
        file_list = [
            FileInfo(
                path=f.path,
        ...
        logger.debug(f"final file list: {file_list}")
        breakpoint()
        self.create_recover_file(file_list, voice_path, download_order)
        ...
        recover = [
            {
                ...
                "url": file.url,
                "should_download": 1 <= file.should_download <= disable_order,
                "fileId": file.id,
            }
