...
@click.option('--save/--no-save', '-s/-ns', is_flag=True, default=True, help='should your change to the file be saved')
def sql(sql_name: str, save: bool = False):
    """execute a sql statement by sql file name in `sqls` directory and print the results on your terminal"""
    sql_path = Path(__file__).parent.parent / 'sqls' / sql_name
    sql_path = sql_path.with_suffix('.sql')
    if not sql_path.exists():
        logger.error(f"cannot locate sql {sql_name}, check ./sqls for possible sql scripts")
        return
    ...
    if save:
        sql_path.write_text(temp_file_path.read_text(encoding='utf8'), encoding='utf8')
